...
public class ScatteringBullet extends Thread {
	private XJLabel xjl;
	private JPanel jp;
	...
	public ScatteringBullet(XJLabel xjl, LevelMap lm, String name) {
		this.xjl = xjl;
		this.jp = lm.getJp();
		this.name = name;
		this.lm = lm;
	...
	class BulletMove extends Thread {
		...
		public void run() {
			...
			while (x > 0 && x < jp.getWidth() && y > 0 && y < jp.getHeight()
