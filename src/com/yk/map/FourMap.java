...
public class FourMap extends Thread implements LevelMap {
	...
	public void run() {
		...
		if (!StringOperation.strWho(new UserInfo().getGrade(), "5")) {
			try {// 起始剧情
				Thread.sleep(500);
	...
	class endploat extends Thread {
		...
		public void run() {
			...
			if (!StringOperation.strWho(new UserInfo().getGrade(), "5")
				...
				try {// 结束剧情
					...
					while (User.getJl().getY() > 0) {
						User.getJl().setLocation(User.getJl().getX(),
								User.getJl().getY() - 8);
						Thread.sleep(5);
					}
					/*
					...
					uset.setGrade("6");
					uset.setHp(StringOperation.strAdd(uset.getHp(), "100"));
					uset.setAp(StringOperation.strAdd(uset.getAp(), "10"));
					uset.setStorage();
			...
			if (StringOperation.strWho(User.hp, "0")) {
				int ran = new Random().nextInt(10) % 3;
				if (ran == 0) {
					UserSetInfo uset = new UserSetInfo();
					uset.setHp(StringOperation.strAdd(uset.getHp(), "5"));
					uset.setStorage();
				} else if (ran == 1) {
					UserSetInfo uset = new UserSetInfo();
