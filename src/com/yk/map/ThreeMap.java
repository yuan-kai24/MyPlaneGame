...
import com.yk.Level.ThreeLevel;
...
public class ThreeMap extends Thread implements LevelMap {
	...
	class endploat extends Thread {
		...
		public void run() {
			...
			ploat.setOpaque(false);
			if (!StringOperation.strWho(new UserInfo().getGrade(), "4") && StringOperation.strWho(User.hp, "0")) {
			...
			if(!StringOperation.strWho(new UserInfo().getHp(), "600") && StringOperation.strWho(User.hp, "0"))
			{
				UserSetInfo uset = new UserSetInfo();
				uset.setHp(StringOperation.strAdd(uset.getHp(), "10"));
				uset.setStorage();
			}
